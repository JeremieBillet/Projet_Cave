...
public class LigneCommandeDAOImpl implements ILigneCommandeDAO{
	...
	public boolean add(LigneCommande pLigneCommande) {
		...
		try {
			...
			ps = this.connection
					.prepareStatement(" INSERT INTO lignes_commandes (alcool_id, commande_id, quantite, prix) VALUES (?,?,?,?)");

			ps.setInt(1, pLigneCommande.getAlcoolId());
			ps.setInt(2, pLigneCommande.getCommandeId());
			ps.setInt(3, pLigneCommande.getQuantite());
			ps.setInt(4, pLigneCommande.getPrix());
	...
	/**
	 * modification d'un produit issu de la ligne de commande (modif de la quantité essentiellement)
	 */
	@Override
	...
	/**
	 * récup d'un produit à partir de id_ligne_commande (id d'insertion du produit dans la ligne de commande)
	 */
	@Override
	public LigneCommande getById(Integer pIdLigneCommande) {
		...
		try {
			...
			return new LigneCommande(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getInt(4), rs.getInt(5));
	...
	/**
	 * récup de tous les produits issus d'une ligne de commande à partir de l'id de commande
	 */
	@Override
...
try {
			...
			while(rs.next()) {
				...
				ligneCommande = new LigneCommande(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getInt(4), rs.getInt(5));
			...
			System.out.println("... (LigneCommandeDAOImpl) Erreur lors de l'excéution de la méthode getAllByIdCommande() ...");
			e.printStackTrace();
